...
function PlusCore.func.GetUserData(cb)
    if not cb then return PlusCore.UserData end
    cb(PlusCore.UserData)
end
...
function PlusCore.func.HasItem(items, amount)
    return exports['qb-inventory']:HasItem(items, amount)
end

-- Utility

function PlusCore.func.DrawText(x, y, width, height, scale, r, g, b, a, text)
...
function PlusCore.func.DrawText3D(x, y, z, text)
    -- Use local function instead
...
function PlusCore.func.RequestAnimDict(animDict)
...
function PlusCore.func.PlayAnim(animDict, animName, upperbodyOnly, duration)
    local flags = upperbodyOnly and 16 or 0
    local runTime = duration or -1
    PlusCore.func.RequestAnimDict(animDict)
    TaskPlayAnim(PlayerPedId(), animDict, animName, 8.0, 1.0, runTime, flags, 0.0, false, false, true)
    RemoveAnimDict(animDict)
end

function PlusCore.func.LoadModel(model)
...
function PlusCore.func.LoadAnimSet(animSet)
...
function PlusCoreDebug(resource, obj, depth)
    TriggerServerEvent('PlusCore:DebugSomething', resource, obj, depth)
end
...
function PlusCore.func.Progressbar(name, label, duration, useWhileDead, canCancel, disableControls, animation, prop, propTwo, onFinish, onCancel)
    if GetResourceState('progressbar') ~= 'started' then error('progressbar needs to be started in order for PlusCore.func.Progressbar to work') end
...
function PlusCore.func.GetVehicles()
    return GetGamePool('CVehicle')
end

function PlusCore.func.GetObjects()
...
function PlusCore.func.GetPeds(ignoreList)
...
function PlusCore.func.GetClosestPed(coords, ignoreList)
    ...
    ignoreList = ignoreList or {}
    local peds = PlusCore.func.GetPeds(ignoreList)
...
function PlusCore.func.IsWearingGloves()
    local ped = PlayerPedId()
...
function PlusCore.func.GetClosestPlayer(coords)
    local ped = PlayerPedId()
    if coords then
        coords = type(coords) == 'table' and vec3(coords.x, coords.y, coords.z) or coords
    else
    ...
    local closestPlayers = PlusCore.func.GetPlayersFromCoords(coords)
    local closestDistance = -1
...
function PlusCore.func.GetPlayersFromCoords(coords, distance)
...
function PlusCore.func.GetClosestVehicle(coords)
    local ped = PlayerPedId()
...
function PlusCore.func.GetClosestObject(coords)
    local ped = PlayerPedId()
...
function PlusCore.func.GetClosestBone(entity, list)
...
function PlusCore.func.GetBoneDistance(entity, boneType, boneIndex)
    local bone
    if boneType == 1 then
...
function PlusCore.func.AttachProp(ped, model, boneId, x, y, z, xR, yR, zR, vertex)
    local modelHash = type(model) == 'string' and GetHashKey(model) or model
    local bone = GetPedBoneIndex(ped, boneId)
    PlusCore.func.LoadModel(modelHash)
...
function PlusCore.func.SpawnVehicle(model, cb, coords, isnetworked, teleportInto)
    local ped = PlayerPedId()
...
function PlusCore.func.DeleteVehicle(vehicle)
...
function PlusCore.func.SpawnClear(coords, radius)
    if coords then
        coords = type(coords) == 'table' and vec3(coords.x, coords.y, coords.z) or coords
    else
...
function PlusCore.func.GetVehicleProperties(vehicle)
...
function PlusCore.func.SetVehicleProperties(vehicle, props)
    if DoesEntityExist(vehicle) then
...
function PlusCore.func.LoadParticleDictionary(dictionary)
...
function PlusCore.func.StartParticleAtCoord(dict, ptName, looped, coords, rot, scale, alpha, color, duration)
    if coords then
        coords = type(coords) == 'table' and vec3(coords.x, coords.y, coords.z) or coords
    else
        coords = GetEntityCoords(PlayerPedId())
    end
    PlusCore.func.LoadParticleDictionary(dict)
...
function PlusCore.func.StartParticleOnEntity(dict, ptName, looped, entity, bone, offset, rot, scale, alpha, color, evolution, duration)
    PlusCore.func.LoadParticleDictionary(dict)
    UseParticleFxAssetNextCall(dict)
...
function PlusCore.func.GetStreetNametAtCoords(coords)
...
function PlusCore.func.GetZoneAtCoords(coords)
    return GetLabelText(GetNameOfZone(coords))
end

function PlusCore.func.GetCardinalDirection(entity)
...
function PlusCore.func.GetCurrentTime()
    local obj = {}
...
function PlusCore.func.GetGroundZCoord(coords)
