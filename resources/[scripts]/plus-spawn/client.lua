...
RegisterNetEvent('qb-spawn:client:openUI', function(value)
    ...
    Wait(1000)
    DoScreenFadeIn(250)
    PlusCore.func.GetPlayerData(function(PlayerData)
...
RegisterNetEvent('qb-spawn:client:setupSpawns', function(cData, new, apps)
    if not new then
        PlusCore.func.TriggerCallback('qb-spawn:server:getOwnedHouses', function(houses)
            local myHouses = {}
            if houses ~= nil then
                for i = 1, (#houses), 1 do
                    myHouses[#myHouses+1] = {
...
RegisterNUICallback('chooseAppa', function(data, cb)
    ...
    TriggerServerEvent('PlusCore:Server:OnPlayerLoaded')
    TriggerEvent('PlusCore:Client:OnPlayerLoaded')
...
RegisterNUICallback('spawnplayer', function(data, cb)
    ...
    if type == "current" then
        PreSpawnPlayer()
        PlusCore.func.GetPlayerData(function(pd)
            ped = PlayerPedId()
