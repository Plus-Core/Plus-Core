...
local function loadSettings(settings)
    ...
    PlusCore.func.Notify(Lang:t("notify.hud_settings_loaded"), "success")
    Wait(1000)
...
local function restartHud()
    ...
    PlusCore.func.Notify(Lang:t("notify.hud_restart"), "error")
    ...
    Wait(2600)
    PlusCore.func.Notify(Lang:t("notify.hud_start"), "success")
end
...
RegisterNetEvent("hud:client:resetStorage", function()
    ...
    PlusCore.func.TriggerCallback('hud:server:getMenu', function(menu) loadSettings(menu); SetResourceKvp('hudSettings', json.encode(menu)) end)
end)
...
RegisterNetEvent("hud:client:LoadMap", function()
    ...
    if Menu.isToggleMapShapeChecked == "square" then
        ...
        if Menu.isMapNotifChecked then
            PlusCore.func.Notify(Lang:t("notify.load_square_map"))
        end
        ...
        Wait(1200)
        if Menu.isMapNotifChecked then
            PlusCore.func.Notify(Lang:t("notify.loaded_square_map"))
        end
    elseif Menu.isToggleMapShapeChecked == "circle" then
        ...
        if Menu.isMapNotifChecked then
            PlusCore.func.Notify(Lang:t("notify.load_circle_map"))
        end
        ...
        Wait(1200)
        if Menu.isMapNotifChecked then
            PlusCore.func.Notify(Lang:t("notify.loaded_circle_map"))
        end
    end
end)
...
RegisterNUICallback('cinematicMode', function(_, cb)
    ...
    if Menu.isCineamticModeChecked then
        ...
        if Menu.isCinematicNotifChecked then
            PlusCore.func.Notify(Lang:t("notify.cinematic_off"), 'error')
        end
        DisplayRadar(1)
    else
        CinematicShow(true)
        ...
        if Menu.isCinematicNotifChecked then
            PlusCore.func.Notify(Lang:t("notify.cinematic_on"))
        end
    end
    TriggerEvent("hud:client:playHudChecklistSound")
    saveSettings()
...
CreateThread(function()
    ...
    while true do
        ...
        if LocalPlayer.state.isLoggedIn then
            ...
            playerDead = IsEntityDead(player) or UserData.metadata["inlaststand"] or UserData.metadata["isdead"] or false
            ...
            if IsPedInAnyVehicle(ped, false) and not IsThisModelABicycle(GetEntityModel(GetVehiclePedIsIn(ped, false))) then
                if exports['LegacyFuel']:GetFuel(GetVehiclePedIsIn(ped, false)) <= 20 then -- At 20% Fuel Left
                    if Menu.isLowFuelChecked then
                        ...
                        PlusCore.func.Notify(Lang:t("notify.low_fuel"), "error")
...
RegisterNetEvent('hud:client:OnMoneyChange', function(type, amount, isMinus)
    cashAmount = UserData.money['cash']
    bankAmount = UserData.money['bank']
    SendNUIMessage({
        action = 'updatemoney',
        cash = cashAmount,
        bank = bankAmount,
