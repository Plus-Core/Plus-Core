...
local function updatePermissionLevel()
    PlusCore.func.TriggerCallback('qb-afkkick:server:GetPermissions', function(userGroups)
                        ...
                        if time then
                            if time > 0 then
                                ...
                                if _type == 'minutes' then
                                    PlusCore.func.Notify(Lang:t('afk.will_kick') .. math.ceil(time / 60) .. Lang:t('afk.time_minutes'), 'error', 10000)
                                elseif _type == 'seconds' then
                                    PlusCore.func.Notify(Lang:t('afk.will_kick') .. time .. Lang:t('afk.time_seconds'), 'error', 10000)
                                end
                                time -= 10
                            else
