            ...
            if Driver == PlayerPed and not washingVehicle then
                if IsControlPressed(0, 38) then
                    if dirtLevel > Config.DirtLevel then
                        ...
                        PlusCore.func.Notify(Lang:t('wash.dirty'), 'error')
                    end
                    listen = false
                    break
                end
...
RegisterNetEvent('qb-carwash:client:washCar', function()
    ...
    local PedVehicle = GetVehiclePedIsIn(PlayerPed, false)
    washingVehicle = true
    PlusCore.func.Progressbar("search_cabin", Lang:t('wash.in_progress'), math.random(4000, 8000), false, true, {
        ...
        PlusCore.func.Notify(Lang:t('wash.cancel'), "error")
        washingVehicle = false
    end)
end)
