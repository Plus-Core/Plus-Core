...
RegisterNetEvent('seatbelt:client:UseHarness', function(ItemData) -- On Item Use (registered server side)
    ...
    if inveh and class ~= 8 and class ~= 13 and class ~= 14 then
        if not harnessOn then
            ...
            PlusCore.func.Progressbar("harness_equip", Lang:t('seatbelt.use_harness_progress'), 5000, false, true, {
            ...
            LocalPlayer.state:set("inv_busy", true, true)
            PlusCore.func.Progressbar("harness_equip", Lang:t('seatbelt.remove_harness_progress'), 5000, false, true, {
                disableMovement = false,
                disableCarMovement = false,
                disableMouse = false,
                disableCombat = true,
        ...
        PlusCore.func.Notify(Lang:t('seatbelt.no_car'), 'error')
    end
end)
