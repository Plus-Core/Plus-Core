...
PlusCore.func.CreateCallback('nos:GetNosLoadedVehs', function(_, cb)
    cb(VehicleNitrous)
end)

PlusCore.Commands.Add("id", "Check Your ID #", {}, false, function(source)
    TriggerClientEvent('QBCore:Notify', source,  "ID: "..source)
end)

PlusCore.func.CreateUseableItem("harness", function(source, item)
...
RegisterNetEvent('equip:harness', function(item)
    ...
    elseif Player.PlayerData.items[item.slot].info.uses == 1 then
        TriggerClientEvent("inventory:client:ItemBox", src, PlusCore.Shared.Items['harness'], "remove")
...
PlusCore.func.CreateCallback('smallresources:server:GetCurrentPlayers', function(_, cb)
