...
local function openMenu(data)
	...
	for _,v in pairs(data) do
		if v["icon"] then
			...
			if PlusCore.Shared.Items[tostring(v["icon"])] then
				if not string.find(PlusCore.Shared.Items[tostring(v["icon"])].image, "images/") then
					img = img.."images/"
				end
				v["icon"] = img..PlusCore.Shared.Items[tostring(v["icon"])].image
			end
		end
	end
        ...
        if data then
            if data.params.event then
                ...
                elseif data.params.isQBCommand then
                    TriggerServerEvent('PlusCore:CallCommand', data.params.event, data.params.args)
