...
CreateThread(function()
    while true do
        ...
        if NetCheck1 or NetCheck2 then
            TriggerServerEvent('plus-admin:server:GetPlayersForBlips')
        end
    end
end)

RegisterNetEvent('plus-admin:client:toggleBlips', function()
    if not ShowBlips then
        ShowBlips = true
        NetCheck1 = true
        PlusCore.func.Notify(Lang:t("success.blips_activated"), "success")
    else
        ShowBlips = false
        PlusCore.func.Notify(Lang:t("error.blips_deactivated"), "error")
    end
end)

RegisterNetEvent('plus-admin:client:toggleNames', function()
    if not ShowNames then
        ShowNames = true
        NetCheck2 = true
        PlusCore.func.Notify(Lang:t("success.names_activated"), "success")
    else
        ShowNames = false
        PlusCore.func.Notify(Lang:t("error.names_deactivated"), "error")
    end
end)

RegisterNetEvent('plus-admin:client:Show', function(players)
