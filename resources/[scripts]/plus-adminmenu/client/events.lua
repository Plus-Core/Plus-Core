...
RegisterNetEvent('plus-admin:client:inventory', function(targetPed)
    TriggerServerEvent("inventory:server:OpenInventory", "otherplayer", targetPed)
end)

RegisterNetEvent('plus-admin:client:spectate', function(targetPed)
...
RegisterNetEvent('plus-admin:client:SendReport', function(name, src, msg)
    TriggerServerEvent('plus-admin:server:SendReport', name, src, msg)
end)
...
RegisterNetEvent('plus-admin:client:SaveCar', function()
    local ped = PlayerPedId()
    local veh = GetVehiclePedIsIn(ped)

    if veh ~= nil and veh ~= 0 then
        local plate = PlusCore.func.GetPlate(veh)
        local props = PlusCore.func.GetVehicleProperties(veh)
        local hash = props.model
        local vehname = getVehicleFromVehList(hash)
        if PlusCore.Shared.Vehicles[vehname] ~= nil and next(PlusCore.Shared.Vehicles[vehname]) ~= nil then
            TriggerServerEvent('plus-admin:server:SaveCar', props, PlusCore.Shared.Vehicles[vehname], GetHashKey(veh), plate)
        else
            PlusCore.func.Notify(Lang:t("error.no_store_vehicle_garage"), 'error')
        end
    else
        PlusCore.func.Notify(Lang:t("error.no_vehicle"), 'error')
    end
end)
...
RegisterNetEvent('plus-admin:client:SetModel', function(skin)
    local ped = PlayerPedId()
...
RegisterNetEvent('plus-admin:client:SetSpeed', function(speed)
    local ped = PlayerId()
    if speed == "fast" then
...
RegisterNetEvent('qb-weapons:client:SetWeaponAmmoManual', function(weapon, ammo)
    ...
    if weapon ~= "current" then
        ...
        PlusCore.func.Notify(Lang:t("info.ammoforthe", {value = ammo, weapon = PlusCore.Shared.Weapons[weapon]["label"]}), 'success')
    else
        ...
        if weapon ~= nil then
            ...
            PlusCore.func.Notify(Lang:t("error.no_weapon"), 'error')
        end
    end
end)

RegisterNetEvent('plus-admin:client:GiveNuiFocus', function(focus, mouse)
...
RegisterNetEvent('plus-admin:client:maxmodVehicle', function()
