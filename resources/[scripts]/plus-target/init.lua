...
CreateThread(function()
	local state = GetResourceState('plus-core')
	if state ~= 'missing' then
		local timeout = 0
		while state ~= 'started' and timeout <= 100 do
			timeout += 1
			state = GetResourceState('plus-core')
			Wait(0)
		end
		Config.Standalone = false
	end
	if Config.Standalone then
		...
		ItemCheck = PlusCore.func.HasItem
		...
		RegisterNetEvent('PlusCore:Client:OnPlayerLoaded', function()
			PlayerData = PlusCore.func.GetPlayerData()
			SpawnPeds()
		end)
		...
		RegisterNetEvent('PlusCore:Player:SetPlayerData', function(val)
			PlayerData = val
		end)
	end
end)
