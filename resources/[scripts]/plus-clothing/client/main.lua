...
local function openMenu(allowedMenus)
    ...
    PlayerData = PlusCore.Functions.GetPlayerData()
...
local function reloadSkin(health)
    ...
    local gender = PlusCore.Functions.GetPlayerData().charinfo.gender
...
local function getOutfits(gradeLevel, data)
    local gender = "male"
    if PlusCore.Functions.GetPlayerData().charinfo.gender == 1 then gender = "female" end
    PlusCore.Functions.TriggerCallback('qb-clothing:server:getOutfits', function(result)
        openMenu({
...
RegisterNetEvent('PlusCore:Client:UpdateObject', function()
	PlusCore = exports['plus-core']:GetCoreObject()
end)
...
AddEventHandler('qb-clothes:client:CreateFirstCharacter', function()
    PlusCore.Functions.GetPlayerData(function(pData)
...
AddEventHandler('qb-clothing:client:loadOutfit', function(oData)
    ...
    if data["accessory"] ~= nil then
        if PlusCore.Functions.GetPlayerData().metadata["tracker"] then
    ...
    if oData.outfitName ~= nil then
        PlusCore.Functions.Notify("You have chosen "..oData.outfitName.."! Press Confirm to confirm outfit.")
    end
end)
RegisterNetEvent("qb-clothing:client:adjustfacewear")
AddEventHandler("qb-clothing:client:adjustfacewear",function(type)
    if PlusCore.Functions.GetPlayerData().metadata["ishandcuffed"] then return end
...
RegisterNetEvent('PlusCore:Client:OnPlayerLoaded', function()
    ...
    PlayerData = PlusCore.Functions.GetPlayerData()
--    PlusCore.Shared.Jobs = exports['qb-jobs']:AddJobs()
end)
RegisterNetEvent('PlusCore:Client:OnJobUpdate', function(JobInfo)
    PlayerData.job = JobInfo
end)
RegisterNetEvent('PlusCore:Client:OnGangUpdate', function(GangInfo)
...
RegisterNUICallback('TrackerError', function(_, cb)
    PlusCore.Functions.Notify("You can't remove your ankle bracelet ..", "error")
    cb('ok')
end)
...
RegisterNUICallback('removeOutfit', function(data, cb)
    ...
    PlusCore.Functions.Notify("You have deleted your"..data.outfitName.." outfit!")
    cb('ok')
end)
RegisterNUICallback('setCurrentPed', function(data, cb)
    local playerData = PlusCore.Functions.GetPlayerData()
...
if Config.UseTarget then
    CreateThread(function()
        ...
        for k, v in pairs(Config.ClothingRooms) do
            ...
            elseif not PlusCore.Shared.QBJobsStatus then
                action = function()
                    customCamLocation = v.cameraLocation
    ...
    CreateThread(function()
        ...
        clothingCombo:onPlayerInOut(function(isPointInside, _, zone)
            if isPointInside then
                ...
                if zoneName == 'surgeon' then
                    exports['plus-core']:DrawText('[E] - Plastic Surgery', 'left')
                elseif zoneName == 'clothing' then
                    exports['plus-core']:DrawText('[E] - Clothing Shop', 'left')
                elseif zoneName == 'barber' then
                    exports['plus-core']:DrawText('[E] - Barber', 'left')
                end
            else
                inZone = false
                exports['plus-core']:HideText()
            end
        end)
        if PlayerData.gang and PlayerData.gang.name then
            ...
            clothingRoomsCombo:onPlayerInOut(function(isPointInside, _, zone)
                if isPointInside then
                    local zoneID = tonumber(PlusCore.Shared.SplitStr(zone.name, "_")[2])
                    local job = Config.ClothingRooms[zoneID].isGang and PlayerData.gang.name or (not PlusCore.Shared.QBJobsStatus and PlayerData.job.name)
            ...
            if inZone then
                ...
                elseif not PlusCore.Shared.QBJobsStatus then
                    if IsControlJustReleased(0, 38) then
                        ...
                        local gradeLevel = clothingRoom.isGang and PlayerData.gang.grade.level or (not PlusCore.Shared.QBJobsStatus and PlayerData.job.grade.level)
